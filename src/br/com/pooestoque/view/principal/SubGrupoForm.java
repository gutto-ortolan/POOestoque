    ...
    private void btnImprimirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnImprimirActionPerformed
        try {
            ...
            if(txfPesquisar.getText().isEmpty() || txfPesquisar.getText() == null ){
                dados = subGrupoCon.getLista();
            }else if(cbxPesquisar.getSelectedIndex() == 1){
                dados = subGrupoCon.getSubGrupoPorID(Integer.parseInt(txfPesquisar.getText()));
            }else if(cbxPesquisar.getSelectedIndex() == 0){
                dados = subGrupoCon.getSubGrupoPorNome(txfPesquisar.getText());
            }else{
                dados = subGrupoCon.getSubGrupoPorSituacao(txfPesquisar.getText());
            }
            
            JRDataSource jRDataSource = new JRBeanCollectionDataSource(dados);
            
            JasperPrint jrPrint = JasperFillManager.fillReport(Teste.class.getResource("SubGrupo.jasper").openStream(), null,  jRDataSource);
