 ...
 */
public class OrigemProdutoForm extends javax.swing.JInternalFrame {

    private DefaultTableModel tabelaModelo;
    private Integer sizeTab = 1145;
    private OrigemProdutoCon origemProdutoCon = new OrigemProdutoCon();

    private static OrigemProdutoForm testeframe;

    public static OrigemProdutoForm getInstancia() {
        if (testeframe == null) {
            testeframe = new OrigemProdutoForm();
        }
        return testeframe;
    }

    public OrigemProdutoForm() {
        initComponents();
        setResizable(false);
    ...
    public void readJTable() {
        ...
        for (OrigemProduto origemProduto : origemProdutoCon.getLista()) {
            tabelaModelo.addRow(new Object[]{
                origemProduto.getIdOrigem(),
                origemProduto.getDsOrigem(),
                origemProduto.getStOrigem()
            });
        }
    }
    ...
    public void readJTableForDescricao() {
        ...
        for (OrigemProduto origemProduto : origemProdutoCon.getOrigemProdutoPorNome(txfPesquisar.getText().toUpperCase())) {
            tabelaModelo.addRow(new Object[]{
                origemProduto.getIdOrigem(),
                origemProduto.getDsOrigem(),
                origemProduto.getStOrigem()
    ...
    public void readJTableForID() {
        ...
        for (OrigemProduto origemProduto : origemProdutoCon.getOrigemProdutoPorID(Integer.parseInt(txfPesquisar.getText()))) {
            tabelaModelo.addRow(new Object[]{
                origemProduto.getIdOrigem(),
                origemProduto.getDsOrigem(),
                origemProduto.getStOrigem()
    ...
    public void readJTableForSituacao() {
        ...
        for (OrigemProduto origemProduto : origemProdutoCon.getOrigemProdutoPorSituacao(txfPesquisar.getText())) {
            tabelaModelo.addRow(new Object[]{
                origemProduto.getIdOrigem(),
                origemProduto.getDsOrigem(),
                origemProduto.getStOrigem()
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setIconifiable(true);
        setTitle("Manutenção de Origem Produto");
    ...
    private void bntNovoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntNovoActionPerformed
        ...
        janelaNovo.setTitle("Nova Origem Produto");
        janelaNovo.setModal(true);
        janelaNovo.setLocationRelativeTo(null);
        janelaNovo.setVisible(true);

        OrigemProduto origemProduto = janelaNovo.getNovoOrigemProduto();
    ...
    private void btnExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExcluirActionPerformed
        if (tabela.getSelectedRow() != -1) {
            ...
            if (JOptionPane.showConfirmDialog(null, "Deseja realmente excluir?", "", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION) {
                OrigemProduto obj = origemProdutoCon.getOrigemProduto(Integer.parseInt(tabela.getValueAt(tabela.getSelectedRow(), 0).toString()));
                origemProdutoCon.excluir(obj);
    ...
    private void btnImprimirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnImprimirActionPerformed
        try {
            ...
            if(txfPesquisar.getText().isEmpty() || txfPesquisar.getText() == null ){
                dados = origemProdutoCon.getLista();
            }else if(cbxPesquisar.getSelectedIndex() == 1){
                dados = origemProdutoCon.getOrigemProdutoPorID(Integer.parseInt(txfPesquisar.getText()));
            }else if(cbxPesquisar.getSelectedIndex() == 0){
                dados = origemProdutoCon.getOrigemProdutoPorNome(txfPesquisar.getText());
            }else{
                dados = origemProdutoCon.getOrigemProdutoPorSituacao(txfPesquisar.getText());
            }
            
            JRDataSource jRDataSource = new JRBeanCollectionDataSource(dados);
            
            JasperPrint jrPrint = JasperFillManager.fillReport(Teste.class.getResource("OrigemProduto.jasper").openStream(), null,  jRDataSource);
