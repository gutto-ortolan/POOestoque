...
public class TipoProdutoForm extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setIconifiable(true);
        setTitle("Manutenção de Tipo Produto");
    ...
    private void bntNovoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntNovoActionPerformed
        NovoGeralForm janelaNovo = new NovoGeralForm("Tipo", "Novo");
        janelaNovo.setTitle("Novo Tipo Produto");
        janelaNovo.setModal(true);
        janelaNovo.setLocationRelativeTo(null);
        janelaNovo.setVisible(true);

        TipoProduto tipoProduto = janelaNovo.getNovoTipoProduto();
        if (tipoProduto != null) {
    ...
    private void btnAlterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAlterarActionPerformed
        if (tabela.getSelectedRow() != -1) {
           TipoProduto tipoProduto = tipoProdutoCon.getTipoProduto((Integer) tabela.getValueAt(tabela.getSelectedRow(), 0));

            NovoGeralForm janela = new NovoGeralForm("Tipo", "Altera") {
            };
            janela.setTitle("Alterar Tipo Produto");
            janela.setModal(true);
            janela.setLocationRelativeTo(null);
            janela.setTipoProdutoAlterar(tipoProduto);
    ...
    private void btnExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExcluirActionPerformed
        if (tabela.getSelectedRow() != -1) {
            ...
            if (JOptionPane.showConfirmDialog(null, "Deseja realmente excluir?", "", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION) {
                TipoProduto obj = tipoProdutoCon.getTipoProduto(Integer.parseInt(tabela.getValueAt(tabela.getSelectedRow(), 0).toString()));
                tipoProdutoCon.excluir(obj);
    ...
    private void btnImprimirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnImprimirActionPerformed
        try {
            ...
            if(txfPesquisar.getText().isEmpty() || txfPesquisar.getText() == null ){
                dados = tipoProdutoCon.getLista();
            }else if(cbxPesquisar.getSelectedIndex() == 1){
                dados = tipoProdutoCon.getTipoProdutoPorID(Integer.parseInt(txfPesquisar.getText()));
            }else if(cbxPesquisar.getSelectedIndex() == 0){
                dados = tipoProdutoCon.getTipoProdutoPorNome(txfPesquisar.getText());
            }else{
                dados = tipoProdutoCon.getTipoProdutoPorSituacao(txfPesquisar.getText());
            }
            
            JRDataSource jRDataSource = new JRBeanCollectionDataSource(dados);
            
            JasperPrint jrPrint = JasperFillManager.fillReport(Teste.class.getResource("TipoProduto.jasper").openStream(), null,  jRDataSource);
