...
import br.com.pooestoque.view.adicionar.NovoFornecedorForm;
...
public class FornecedorForm extends javax.swing.JInternalFrame {
    ...
    private static FornecedorForm testeframe;

    public static FornecedorForm getInstancia() {
        if (testeframe == null) {
            testeframe = new FornecedorForm();
        }
        return testeframe;
    }
    ...
    public FornecedorForm() {
        initComponents();
        setResizable(false);
        tabela.setModel((new DefaultTableModel(null, new Object[]{"Código", "Nome", "Cnpj", "Visita", "Situação"})));
        tamanho_colunas();
    }
    ...
    private void bntNovoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntNovoActionPerformed
        NovoFornecedorForm janelaNovo = new NovoFornecedorForm("Novo") {
        };
        janelaNovo.setTitle("Novo Fornecedor");
        janelaNovo.setModal(true);
        janelaNovo.setLocationRelativeTo(null);
        janelaNovo.setVisible(true);

        Fornecedor fornecedor = janelaNovo.getNovoFornecedor();
        if (fornecedor != null) {
    ...
    private void btnAlterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAlterarActionPerformed
        if (tabela.getSelectedRow() != -1) {
            Fornecedor fornecedor = fornecedorCon.getFornecedor((Integer) tabela.getValueAt(tabela.getSelectedRow(), 0));

            NovoFornecedorForm janela = new NovoFornecedorForm("Novo") {
        };
            janela.setTitle("Alterar Fornecedor");
            janela.setModal(true);
            janela.setLocationRelativeTo(null);
            janela.setFornecedorAlterar(fornecedor);
    ...
    private void btnImprimirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnImprimirActionPerformed
        try {
            ...
            if(txfPesquisar.getText().isEmpty() || txfPesquisar.getText() == null ){
                dados = fornecedorCon.getLista();
            }else if(cbxPesquisar.getSelectedIndex() == 1){
                dados = fornecedorCon.getFornecedorPorID(Integer.parseInt(txfPesquisar.getText()));
            }else if(cbxPesquisar.getSelectedIndex() == 0){
                dados = fornecedorCon.getFornecedorPorNome(txfPesquisar.getText());
            }else{
                dados = fornecedorCon.getFornecedorPorSituacao(txfPesquisar.getText());
            }
            
            JRDataSource jRDataSource = new JRBeanCollectionDataSource(dados);
            
            JasperPrint jrPrint = JasperFillManager.fillReport(Teste.class.getResource("Fornecedor.jasper").openStream(), null,  jRDataSource);
    ...
    private void tamanho_colunas() {
        ...
        modeloDaColuna.getColumn(2).setMaxWidth((sizeTab * 25) / 100);
        modeloDaColuna.getColumn(3).setMaxWidth((sizeTab * 15) / 100);
        modeloDaColuna.getColumn(4).setMaxWidth((sizeTab * 15) / 100);
