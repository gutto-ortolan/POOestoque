...
public class DivisaoForm extends javax.swing.JInternalFrame {
    ...
    private static DivisaoForm testeframe;

    public static DivisaoForm getInstancia() {
        if (testeframe == null) {
            testeframe = new DivisaoForm();
        }
        return testeframe;
    }
    ...
    public DivisaoForm() {
        initComponents();
        setResizable(false);
        tabela.setModel((new DefaultTableModel(null, new Object[]{"Código", "Descrição", "SubGrupo", "Situação"})));
        tamanho_colunas();
    }
    ...
    public void readJTable() {
        ...
        for (Divisao divisao : divisaoCon.getLista()) {
    ...
    public void readJTableForDescricao() {
        ...
        for (Divisao divisao : divisaoCon.getDivisaoPorNome(txfPesquisar.getText().toUpperCase())) {
            tabelaModelo.addRow(new Object[]{
                divisao.getIdDivisao(),
                divisao.getDsDivisao(),
                divisao.getSubGrupo().getDsSubGrupo(),
    ...
    public void readJTableForID() {
        ...
        for (Divisao divisao : divisaoCon.getDivisaoPorID(Integer.parseInt(txfPesquisar.getText()))) {
            tabelaModelo.addRow(new Object[]{
                divisao.getIdDivisao(),
                divisao.getDsDivisao(),
                divisao.getSubGrupo().getDsSubGrupo(),
    ...
    public void readJTableForSituacao() {
        ...
        for (Divisao divisao : divisaoCon.getDivisaoPorSituacao(txfPesquisar.getText())) {
            tabelaModelo.addRow(new Object[]{
                divisao.getIdDivisao(),
                divisao.getDsDivisao(),
                divisao.getSubGrupo().getDsSubGrupo(),
    ...
    private void btnImprimirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnImprimirActionPerformed
        try {
            ...
            if(txfPesquisar.getText().isEmpty() || txfPesquisar.getText() == null ){
                dados = divisaoCon.getLista();
            }else if(cbxPesquisar.getSelectedIndex() == 1){
                dados = divisaoCon.getDivisaoPorID(Integer.parseInt(txfPesquisar.getText()));
            }else if(cbxPesquisar.getSelectedIndex() == 0){
                dados = divisaoCon.getDivisaoPorNome(txfPesquisar.getText());
            }else{
                dados = divisaoCon.getDivisaoPorSituacao(txfPesquisar.getText());
            }
            
            JRDataSource jRDataSource = new JRBeanCollectionDataSource(dados);
            
            JasperPrint jrPrint = JasperFillManager.fillReport(Teste.class.getResource("Divisao.jasper").openStream(), null,  jRDataSource);
